...
st.title("🩺 Medical Report Generator with PDF and Audio")
...
def generate_detailed_report(name, age, mobile, query, df):
    ...
    return report_text, disease, medicine
...
# TTS Generation for Disease and Medicine
def generate_audio_tts(disease, medicine, lang_code, filename="audio.mp3"):
    text = f"The diagnosed disease is {disease} and the medicine prescribed is {medicine}."
...
if st.button("Generate Report"):
    if name and mobile and query:
        report, disease, medicine = generate_detailed_report(name, age, mobile, query, df)
        if report:
            ...
            # Audio generation for disease and medicine
            audio_filename = "audio.mp3"
            lang_code = lang_code_map[language]
            generate_audio_tts(disease, medicine, lang_code, audio_filename)
